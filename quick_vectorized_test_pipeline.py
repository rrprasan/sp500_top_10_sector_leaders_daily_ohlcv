#!/usr/bin/env python3
"""
Quick Vectorized Scanner Test Pipeline (1-Hour Version)
Downloads OHLCV data from Aug 1, 2023 to Aug 29, 2025 for top 11 tickers
to test Snowflake VECTORIZED_SCANNER performance in ~1 hour.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('quick_vectorized_test_pipeline.log'),
...
class QuickVectorizedTestPipeline:
    """
    Quick 1-hour pipeline for testing Snowflake VECTORIZED_SCANNER performance.
    Downloads data for top 11 tickers from Aug 1, 2023 to Aug 29, 2025.
    """
    ...
    def __init__(self):
        ...
        # Top 11 tickers for quick test (major companies from different sectors)
        self.TEST_TICKERS = [
            'AAPL',   # Technology
            'MSFT',   # Technology  
        ...
        logger.info("Quick Vectorized Scanner Test Pipeline initialized")
    ...
    def run(self) -> None:
        """Run the quick vectorized scanner test pipeline."""
        logger.info("=" * 80)
        logger.info("QUICK VECTORIZED SCANNER TEST PIPELINE (1-HOUR VERSION)")
        logger.info("=" * 80)
        logger.info(f"Date range: {self.START_DATE} to {self.END_DATE}")
        logger.info(f"Test tickers: {len(self.TEST_TICKERS)} tickers")
        ...
        try:
            ...
            # Step 2: Process each test ticker
            logger.info(f"Step 2: Processing {len(self.TEST_TICKERS)} test tickers...")
            successful_tickers = 0
            failed_tickers = []
            
            # Calculate estimated time
            total_months = len(self._generate_date_ranges())
            total_api_calls = len(self.TEST_TICKERS) * total_months
            ...
            if successful_tickers >= 8:  # At least 8 out of 11 tickers
                logger.info("🎉 Sufficient data collected for vectorized scanner testing!")
                logger.info(f"S3 bucket contains {successful_tickers} Parquet files ready for Snowflake COPY command.")
                logger.info("")
                logger.info("Next steps:")
                logger.info("1. Run: python test_vectorized_scanner.py")
                logger.info("2. Compare COPY INTO performance with USE_VECTORIZED_SCANNER = FALSE vs TRUE")
                logger.info("3. Analyze performance metrics and recommendations")
            else:
                logger.warning(f"Only {successful_tickers} tickers successful - may need more data for reliable testing")
            
            logger.info("Quick Vectorized Scanner Test Pipeline completed")
...
def main():
    ...
    try:
        ...
        pipeline = QuickVectorizedTestPipeline()
        pipeline.run()
